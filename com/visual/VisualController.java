    ...
    public void generateMaze(Maze maze, JPanel container, Color pathColor) {
        ...
        for (int i = 0; i < template.length; i++) {
            for (int j = 0; j < template[0].length; j++) {
                ...
                {
                    vertical.setBounds(horizontalOffset + unitSizeWidth,verticalOffset, unitSizeWidth, unitSizeHeight * 3);
                }
                else if(template[i][j].isnPath())
                {
                    vertical.setBounds(horizontalOffset + unitSizeWidth, verticalOffset, unitSizeWidth, unitSizeHeight * 2);
                }
                else if(template[i][j].issPath())
                {
                    vertical.setBounds(horizontalOffset + unitSizeWidth, verticalOffset + unitSizeHeight, unitSizeWidth, unitSizeHeight * 2);
                }
                //check the horizontal direction
                MazeCube horizontal = new MazeCube(pathColor);
                if(template[i][j].isePath() && template[i][j].iswPath())
                {
                    horizontal.setBounds(horizontalOffset,verticalOffset + unitSizeHeight, unitSizeWidth * 3, unitSizeHeight);
                }
                else if(template[i][j].isePath())
                {
                    horizontal.setBounds(horizontalOffset + unitSizeWidth, verticalOffset + unitSizeHeight, unitSizeWidth * 2, unitSizeHeight);
                }
                else if(template[i][j].iswPath())
                {
                    horizontal.setBounds(horizontalOffset, verticalOffset + unitSizeHeight, unitSizeWidth * 2, unitSizeHeight);
                }
